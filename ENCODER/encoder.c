...
__speed speed= {0};
__speed_desir speed_desired = {0};
__pid pid = {0};
__error_p prev_error = {0};
__pid_i i_pid = {0};
...
void initI2C(){
    ...
    // configura 4 encoder
    gpio_set_function(ENCODER_I2C_SDA_PIN_4, GPIO_FUNC_NULL);
    gpio_set_function(ENCODER_I2C_SCL_PIN_4, GPIO_FUNC_NULL);
     // coloca en pull up 
    gpio_pull_up(ENCODER_I2C_SDA_PIN_4);
    gpio_pull_up(ENCODER_I2C_SCL_PIN_4);
    ...
    bi_decl(bi_2pins_with_func(ENCODER_I2C_SDA_PIN_4, ENCODER_I2C_SCL_PIN_4, GPIO_FUNC_I2C));
...
int main() {
    ...
    while(true){
        ...
        if(((current_micros - ayudamedios)) >= 1500){
            ...
            switch (encoder){
                case 0:
                    obtainAngle(i2c0, startAngle.startAngle_1);
                    break;
                case 1:
                    obtainAngle(i2c1, startAngle.startAngle_3);
                    break;
                case 2:
                    obtainAngle(i2c0, startAngle.startAngle_2);
                    break;
                case 3:
                    obtainAngle(i2c1, startAngle.startAngle_4);
                    break;
                default:
                    break;
                }
